...
def load_rgb_as_array(dsm_path, color = 'r'):
    """
    ...
    print(f"Loaded DSM with shape: {r_array.shape}")
    if color == 'r':
        return r_array
    elif color == 'g':
        return g_array
...
def apply_canny_otsu(dsm_array, sigma_prefilter, sigma=1.0, low_threshold=None, high_threshold=None, debug=False):
    """
    ...
    if sigma_prefilter is not None : 
    ...
    edges = morphology.binary_dilation(edges, morphology.disk(4))  # Thicken remaining edges
    edges = morphology.binary_erosion(edges, morphology.disk(3))  # Clean up
    # # 4. Remove very small objects
    edges = morphology.remove_small_objects(edges, min_size=1000)
...
def process_rgb_with_filter(rgb_path, output_dir, color='r', sigma_prefilter=2, filter_func=None, filter_name=None,
    ...
    base_filename = os.path.splitext(os.path.basename(rgb_path))[0]

    # Load DSM
    dsm_array = load_rgb_as_array(rgb_path, color=color)  # Crop to a smaller area for testing
    ...
    if save_tif:
        ...
        save_raster_as_tif(filtered_array, rgb_path, output_tif_path)
...
def main():
    ...
    rgb_path = "drone_treated/WAP32_tiles/rgb/WAP32_full_transparent_mosaic_group1_09_03.tif"
    output_dir = "data/cany_otsu/rgb"
    ...
    for sigma_prefilter in [70]:
        for sigma in[100]:
            canny_otsu_kwargs = {'sigma': sigma, 'sigma_prefilter': sigma_prefilter, 'debug': False}
            process_rgb_with_filter( 
                color='b',
                rgb_path=rgb_path,
                output_dir=output_dir,
                sigma_prefilter=sigma_prefilter,
                filter_func=apply_canny_otsu,
                filter_name=f"canny_otsu_sigma{sigma}_prefilter{sigma_prefilter}",
