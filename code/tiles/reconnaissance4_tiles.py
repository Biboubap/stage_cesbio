...
def get_available_features(samples, exclude_temp=False, exclude_large=False):
    """
    ...
    Args:
        ...
        exclude_large: Si True, exclut les features de voisinage large
    """
    ...
    # Si on exclut les features de voisinage large
    if exclude_large:
        large_features = ["r_large_mean", "g_large_mean", "b_large_mean", "t_large_mean", "z_moins_z_large"]
        all_features = [f for f in all_features if f not in large_features]
...
def load_samples(json_path, exclude_temp=False, exclude_large=False):
    with open(json_path) as f:
        data = json.load(f)
    samples = data["samples"]
    feature_names = get_available_features(samples, exclude_temp, exclude_large)
    features = []
    labels = []
    for s in samples:
        feat = []
...
if __name__ == "__main__":
    # Option pour exclure les features de voisinage large
    exclude_large = True
    
    features, labels, feature_names = load_samples("data/samples/selection10/pop8_converted.json", 
                                                 exclude_temp=True, 
                                                 exclude_large=exclude_large)
    
    print(f"Nombre de features utilisées: {len(feature_names)}")
    ...
    import joblib
    model_filename = "data/samples/selection10/model10"
    if exclude_large:
        model_filename += "_no_large"
    model_filename += ".joblib"
    
    joblib.dump({"model": clf, "feature_names": feature_names}, model_filename)
