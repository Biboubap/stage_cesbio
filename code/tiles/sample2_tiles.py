...
class Sample2:
    ...
    def compute_neighbors_all(self, sample_set, distance_large=3):
        ...
        Args:
            sample_set: Ensemble de samples contenant éventuellement les voisins
        ...
        # Si distance_large est 0, on saute ce calcul
        if distance_large > 0:
                ...
                for j in range(-distance_large, distance_large + 1):
                    ...
                    if neighbor is not None:
                        ...
                        if has_t and stats['t_mean'] is not None:
                            tL_sum += stats['t_mean']
                        nb_neighborsL += 1
                        stats = None  # Clean up stats to free memory
            ...
            # Si distance_large est 0, on met tous ces attributs à None
            self.r_large_mean = self.g_large_mean = self.b_large_mean = None
            self.t_large_mean = None
        ...
        if has_z and distance_large > 0:
            ...
            # Si distance_large est 0 ou pas de Z, on met cet attribut à None
            self.z_moins_z_large = None

        # Final garbage collection to ensure all temporary objects are freed
        gc.collect()
