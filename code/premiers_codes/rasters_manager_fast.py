...
class RastersManager:
    ...
    def calculate_patch_stats(self, x, y, size_patch):
        ...
        # RGB statistics - use optimized numpy operations
        if self.rast_r is not None and self.rast_g is not None and self.rast_b is not None:
            # Extract views into the arrays instead of copying data
    ...
    def calculate_patches_stats_batch(self, positions, size_patch):
        """
        Calculate statistics for multiple patches in a single batch operation.
        
        Args:
            positions: List of (x, y) tuples representing patch positions
            size_patch: Size of each patch
            
        Returns:
            List of dictionaries with statistics for each patch
        """
        if not self.is_loaded:
            self.load_rasters()
        ...
        # Process all positions in one batch
        for x, y in positions:
            stats = self.calculate_patch_stats(x, y, size_patch)
            results.append(stats)
            
        return results
    
    def calculate_mean_stats_batch(self, positions, size_patch):
        """
        Calculate only mean statistics for multiple patches in batch.
        More memory efficient when only means are needed.
        ...
        Returns:
            Dictionary with lists of mean values for r, g, b, z, t
        """
        if not self.is_loaded:
            self.load_rasters()
        ...
        has_rgb = self.rast_r is not None and self.rast_g is not None and self.rast_b is not None
        has_z = self.rast_z is not None
        has_t = self.rast_t is not None
        
        # Process all positions efficiently
        for x, y in positions:
            if has_rgb:
                r_means.append(float(np.mean(self.rast_r[x:x + size_patch, y:y + size_patch])))
                g_means.append(float(np.mean(self.rast_g[x:x + size_patch, y:y + size_patch])))
                b_means.append(float(np.mean(self.rast_b[x:x + size_patch, y:y + size_patch])))
                
            if has_z:
                z_means.append(float(np.mean(self.rast_z[x:x + size_patch, y:y + size_patch])))
                
            if has_t:
                t_means.append(float(np.mean(self.rast_t[x:x + size_patch, y:y + size_patch])))
        
        # Return results in a dictionary
        return {
            'r_means': r_means if has_rgb else None,
            'g_means': g_means if has_rgb else None,
            'b_means': b_means if has_rgb else None,
            'z_means': z_means if has_z else None,
            't_means': t_means if has_t else None
        }