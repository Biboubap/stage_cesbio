...
def process_all_tiles(rgb_folder, dsm_folder, out_folder, model1_path, model2_path, 
    ...
    Args:
        ...
        start_row: Optional starting row coordinate (x, vertical/down)
        start_column: Optional starting column coordinate (y, horizontal/right)
        n_workers: Number of Dask workers (None for auto)
        threads_per_worker: Number of threads per worker
    """
    # Corrected coordinate interpretation: x is row (down), y is column (right)
    start_x = start_row if start_row is not None else None  # Row coordinate (vertical/down)
    start_y = start_column if start_column is not None else None  # Column coordinate (horizontal/right)
    ...
    for rgb_file in rgb_files:
        ...
        if match:
            # These are now correctly interpreted as x=row, y=column
            ...
            dsm_patterns = [
                f'*_{x_coord}_{y_coord}.tif',          # Original format
    ...
    # Sort tiles by coordinates (x is row, y is column)
    ...
    if start_x is not None or start_y is not None:
        ...
        for rgb_file, dsm_file, out_file, x_coord, y_coord in tiles_info:
            x_int = int(x_coord)  # Row (down)
            y_int = int(y_coord)  # Column (right)
        ...
        print(f"Filtered from {original_count} to {len(tiles_info)} tiles starting from coordinates (row={start_x_int}, column={start_y_int})")
    ...
    if n_workers is None:
        import psutil
        n_workers = max(1, psutil.cpu_count() - 1)  # Leave one CPU for system processes
    ...
    try:
        # Create delayed function calls for each tile
        delayed_tasks = []
        for i, (rgb_file, dsm_file, out_file, x_coord, y_coord) in enumerate(tiles_info):
            task = delayed(process_single_tile)(
                rgb_file, 
                dsm_file, 
                out_file,
                model1,
        ...
        print(f"Starting parallel processing of {len(delayed_tasks)} tiles...")
...
def process_single_tile(rgb_path, dsm_path, out_path, model1, model2, feature_names_model1, feature_names_model2,
    ...
    Args:
        ...
        x_start, y_start: Starting row and column coordinates
        x_end, y_end: Ending row and column coordinates
        size_patch: Size of patches for classification
    """
    try:
        print(f"Processing tile: {os.path.basename(rgb_path)}")
        
        # Set default bounds if not provided
        if x_end is None or y_end is None:
            ds = gdal.Open(rgb_path)
            if x_end is None:
                x_end = ds.RasterYSize  # Height corresponds to rows (x)
            if y_end is None:
                y_end = ds.RasterXSize  # Width corresponds to columns (y)
            ds = None
        
        # Create samples separately from computing features to handle NaNs
        # n_samples_x = number of samples in the row direction
        # n_samples_y = number of samples in the column direction
        n_samples_x = (x_end - x_start) // size_patch  # Rows
        n_samples_y = (y_end - y_start) // size_patch  # Columns
        ...
        print(f"Error processing tile {os.path.basename(rgb_path)}: {e}")
        import traceback
        traceback.print_exc()
        return None
...
def sort_tiles_by_coordinates(tiles_info):
    ...
    x represents rows (vertical/down direction)
    y represents columns (horizontal/right direction)
    """
    # Convert coordinates to integers for proper numerical sorting
    def get_sort_key(tile_info):
        x_coord = int(tile_info[3])  # Row coordinate (down)
        y_coord = int(tile_info[4])  # Column coordinate (right)
...
if __name__ == "__main__":
    ...
    # More adaptive resource allocation
    num_cpus = max(1, psutil.cpu_count(logical=False))  # Use physical cores
    num_threads = psutil.cpu_count(logical=True)
    thread_per_worker = max(1, num_threads // num_cpus)  # Threads per worker

    total_mem_gb = psutil.virtual_memory().total / (1024**3)
    # Adjust memory percentage based on total available memory
    mem_percent = 0.8 if total_mem_gb > 16 else 0.7 if total_mem_gb > 8 else 0.6
    mem_per_worker = max(2, min(16, total_mem_gb / num_cpus * mem_percent))
    
    print(f"Using up to {num_cpus} workers with approximately {mem_per_worker:.1f}GB memory per worker")
